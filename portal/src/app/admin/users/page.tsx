...
export default async function PortalUsersPage() {
  ...
  const [users, roles] = await Promise.all([
    apiFetch<PaginatedResult<PortalUserSummary>>("/portal-users?page=1&pageSize=10", {
      cache: "no-store",
      fallbackValue: fallback,
      suppressError: true,
      onError: (error) => console.warn("[portal-users] Không thể tải danh sách portal users", error),
    }),
    apiFetch<PortalRoleSummary[]>("/portal-roles", {
      cache: "no-store",
      fallbackValue: [],
      suppressError: true,
      onError: (error) => console.warn("[portal-users] Không thể tải danh sách role", error),
    }),
  ]);
  ...
  return (
    <div className="space-y-6">
      ...
      />
      <PortalUserManager initialUsers={users} roles={roles} />
    </div>
  );
}
